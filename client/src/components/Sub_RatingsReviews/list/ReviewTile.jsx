...
export default function ReviewTile({ review, handleHelpfulClick }) {
  /**
   * States
   * @type {boolean}
   * @type {function}
  ...
  /** @type {string} */
  const summary =
    review.summary.length > maxSummaryLength
    ...
    <li aria-label="an individual review" className="review-tile">
      <div className="stars-name-date-container">
        ...
        <div className="review-top-corner">
          <p aria-label="reviewer username" className="reviewer-name">
            {review.reviewer_name}
          </p>
          <small aria-label="review timestamp" className="review-date">
            {date}
          </small>
        </div>
      </div>

      <b aria-label="review summary" className="review-summary">
        {summary}
      </b>
      <p aria-label="review body" className="review-body">
        {!showMoreBody ? review.body.slice(0, defaultBodyLength) : review.body}
      </p>
      {review.body.length > defaultBodyLength && !showMoreBody ? (
        <button
          aria-label="show more review body text"
          type="button"
          onClick={() => setShowMoreBody(true)}
        >
          Show More
        </button>
      ) : null}
      ...
      {review.recommend && (
        <small
          aria-label="user recommends product"
      ...
      {review.response && (
        <p aria-label="response from seller" className="review-response">
